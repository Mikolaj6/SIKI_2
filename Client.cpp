...
void do_upload(std::string rest) {
    ...
    if(is) {
        BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} File is successfully opened, name: " << fs::path(rest).filename().string() << " size: " << fs::file_size(fs::path(rest));
    } else {
        outputMutex.lock();
    ...
    for(auto servInfo: setServerSpace) {
        // Send request
        BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} name:" << servInfo.first << " size: " << servInfo.second;

        uint64_t specialSeq = sendComplex(UDPSocket, CMD_PORT, servInfo.first, "ADD\0\0\0\0\0\0\0", fs::file_size(filePath), filePath.filename().string(), true);
        ...
        // Check if data was valid (length < 0) + return on timeout
        int test = clientValidateRecv(length, "UPLOAD", false);
        if (test == -1|| test == 0) {
            continue;
        }

        SIMPL_CMD replySim = *((SIMPL_CMD *) &reply);

        // Special Verification Can be either NO_WAY, or CAN_ADD
        if(customStrCheck(reply.cmd, "CAN_ADD\0\0\0")) {
            BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} length: " << length << " CAN_ADD";
            if(!verifyReceived(server_address, false, be64toh(reply.cmd_seq) != specialSeq, length != 26, strcmp(replySim.data, "") != 0))
                continue;
        } else if(customStrCheck(replySim.cmd, "NO_WAY\0\0\0\0")) {
            BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} length: " << length << " NO_WAY";
            verifyReceived(server_address, false, be64toh(replySim.cmd_seq) != specialSeq, length <= 18, strcmp(replySim.data, filePath.filename().string().c_str()) != 0);
            continue;
        } else {
            verifyReceived(server_address, true, false, false, false);
            continue;
        }
        ...
        if(sendFileClient(tcpSocket, rest)){
            outputMutex.lock();
            std::cout << "File " << fs::path(rest).filename().string() << " uploaded (" << inet_ntoa(server_address.sin_addr) << ":" << ntohs(server_address.sin_port) << ")" << std::endl;
            outputMutex.unlock();
            close(tcpSocket);
            return;
        } else {
            outputMutex.lock();
            std::cout << "File " << fs::path(rest).filename().string() << " uploading failed (" << inet_ntoa(server_address.sin_addr) << ":" << ntohs(server_address.sin_port) << ") Error in sendto" << std::endl;
            outputMutex.unlock();
            close(tcpSocket);
        }
        close(tcpSocket);
    ...
    outputMutex.lock();
    std::cout << "File " << fs::path(rest).filename().string() << " too big" << std::endl;
    outputMutex.unlock();
}
...
void do_fetch(std::string rest) {
    ...
    if(forFetch.find(rest) == forFetch.end()) {
        ...
        outputMutex.unlock();
        sharedMutexForFetch.unlock_shared();
        return;
    } else {
    ...
    // Check if data was valid (length < 0) + return on timeout
    int test = clientValidateRecv(length, "FETCH", false);
    if (test == -1) {
        outputMutex.lock();
        std::cout << "File " << rest << " downloading failed (:) Bad read" << std::endl;
        outputMutex.unlock();
        return;
    } else if (test == 0) {
        outputMutex.lock();
        std::cout << "File " << rest << " downloading failed (:) Timeout, no reply" << std::endl;
        outputMutex.unlock();
        return;
    }
...
void do_search(int &sock, std::string &rest) {
    ...
    while(true) {
        ...
        int test = clientValidateRecv(length, "SEARCH", true);
        if (test == -1) {
            syserr("Bad read");
        } else if (test == 0) {
            continue;
        ...
        BOOST_LOG_TRIVIAL(debug) << "{SEARCH} Received length: " << length;
...
void do_discover(int &sock, bool modeNormal, std::set<std::pair<std::string, uint64_t>, bool (*)(const std::pair<std::string, uint64_t>&, const std::pair<std::string, uint64_t>&)> &setServerSpace) {
    ...
    while(true) {
        ...
        int test = clientValidateRecv(length, "DISCOVER", true);
        if (test == -1) {
            syserr("Bad read");
        } else if (test == 0) {
            continue;
        ...
        if(modeNormal) {
            ...
            BOOST_LOG_TRIVIAL(debug) << "Found " << inet_ntoa(server_address.sin_addr) << " (" << reply.data << ") with free space " << be64toh(reply.param);
...
bool testLine(std::string &line, std::string &&command, std::string &rest, int flag) {
    ...
    if(flag == 1 && line.length() <= command.length() + 1) {
        rest = "";
        return true;
    } else if (flag == 1) {
        return false;
    ...
    if(flag == -1 && line.length() > command.length() + 1) {
        rest = line.substr(command.length() + 1);
        return true;
    } else if (flag == -1) {
        return false;
    }
...
bool sendSomething(void *to_send, int socket, uint32_t length) {
    ...
    if (send(socket, to_send, length, MSG_NOSIGNAL) !=
        ...
        if (errno == EPIPE) { // Sending on closed connection
            BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} Connection lost while sending data";
            return false;
        }
        BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} FailedPartialWrite";
        return false;
    }
    return true;
}
...
// True -> Sending was successful
// False -> there was an error
bool sendFileClient(int socketTCP, std::string filePath) {
    ...
    if (!is) {
        BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} File was empty or sth";
        return false;
    }

    uint64_t fullLength = 0;

    char buffer[client::bufferSize + 1];

    do {
        memset(&buffer, 0, client::bufferSize + 1);
        uint64_t toSendLgh = (uint32_t) is.read(buffer, client::bufferSize).gcount();
        fullLength += toSendLgh;
        if(!sendSomething((void *)buffer, socketTCP, toSendLgh)) {
            return false;
        }
    } while (is);

    BOOST_LOG_TRIVIAL(debug) << "{UPLOAD} Send length: " << fullLength;
...
bool saveFile(int TcpSocket, std::string &filePath) {
    ...
    do {
        ...
        success = readSomething(buff, TcpSocket, client::bufferSize, final);
...
uint64_t sendSimple(int UDPsocket, uint16_t port, std::string servAddr, const char *messageType, std::string dataFieled, bool isClient) {
    ...
    uint32_t extra;
    if(dataFieled.size() > 0)
        extra = dataFieled.size();
    else
        extra = 0;

    if(sendto(UDPsocket, &request, 18 + extra, 0, (struct sockaddr *)&serverSend, sendLen) != (18 + extra)) {
        if(isClient)
            syserr("Bad write");
        else
            return 0;
...
uint64_t sendComplex(int UDPsocket, uint16_t port, std::string servAddr, const char *messageType, uint64_t param, std::string dataFieled, bool isClient) {
    ...
    if(sendto(UDPsocket, &request, 26 + extra, 0, (struct sockaddr *)&serverSend, sendLen) != (26 + extra)) {
        if(isClient)
            syserr("Bad write");
        else
            return 0;
