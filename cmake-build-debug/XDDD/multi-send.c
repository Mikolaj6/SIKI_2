...
//    syserr("bind");
  ...
  for (i = 0; i < 2; ++i) {
    length = strlen("GET TIME");
    sflags = 0;
    snd_len = sendto(sock, "GET TIME", length, sflags,
        (struct sockaddr *) &remote_address, sizeof(remote_address));
    if (snd_len != (ssize_t) length) {
      syserr("partial / failed write");
    }

    rcv_len = recvfrom(sock, buffer, sizeof(buffer), sflags,
        (struct sockaddr *) &remote_address, &rcva_len);
